...
%Date: 30 June 2022
%Purpose: to measure the long and short axis of B. subtilis 168 spores in
%isosmotic and hyperosmotic media

clear, close all

%User Input
dirname = ['/Users/zarina/Downloads/NYU/20220627_Exp1'];
basename_turgid = ['20220627_turgid']; 
basename_hypershock = ['20220627_hypershock'];
labels = {'isosmotic', 'hyperosmotic shock'};
...
troubleshooting = 1; %visualize cells

%% load images and measure spores
[B, centroids, DS, lcell_turgid, mlines, ncells_turgid, pcell, pixels, wcell_turgid] = sporeMeasure(dirname, basename_turgid, dr, lscale, maxA, maxF, minA, minF, sm, troubleshooting);
[B, centroids, DS, lcell_hypershock, mlines, ncells_hypershock, pcell, pixels, wcell_hypershock] = sporeMeasure(dirname, basename_hypershock, dr, lscale, maxA, maxF, minA, minF, sm, troubleshooting);

%% plot the dimensions
figure, hold on
for i = 1:length(ncells_turgid)
    p1 = scatter(lcell_turgid(i, :), wcell_turgid(i, :), 'MarkerFaceColor', okabeIto{1}, 'MarkerEdgeColor', okabeIto{1})
end

for i = 1:length(ncells_hypershock)
    p2 = scatter(lcell_hypershock(i, :), wcell_hypershock(i, :), 'MarkerFaceColor', okabeIto{2}, 'MarkerEdgeColor', okabeIto{2})
end
xlabel('Axis 1 (\mum)')
ylabel('Axis 2 (\mum)')
legend([p1(1), p2(1)], labels, 'Location', 'southeast')

%% Functions
function [B, centroids, DS, lcell, mlines, ncells, pcell, pixels, wcell] = sporeMeasure(dirname, basename, dr, lscale, maxA, maxF, minA, minF, sm, troubleshooting)
